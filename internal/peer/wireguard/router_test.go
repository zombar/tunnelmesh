...
func TestIsWGClientIP(t *testing.T) {
	...
	tests := []struct {
		...
		// WG client range: 10.42.100.0 - 10.42.199.255
		{"10.42.100.1", true},
		{"10.42.100.254", true},
		{"10.42.150.1", true},
		{"10.42.199.254", true},
...
func TestExtractDestIP(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			// IPv4 header: version=4, IHL=5, dest=10.42.100.1
			packet: []byte{
				0x45, 0x00, 0x00, 0x28, // version, IHL, TOS, total length
				0x00, 0x00, 0x00, 0x00, // id, flags, fragment offset
				0x40, 0x06, 0x00, 0x00, // TTL, protocol (TCP), checksum
				0x0a, 0x2a, 0x00, 0x01, // source: 10.42.0.1
				0x0a, 0x2a, 0x64, 0x01, // dest: 10.42.100.1
			},
			want:    "10.42.100.1",
			wantErr: false,
		},
...
func TestExtractSourceIP(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid IPv4 packet",
			// IPv4 header: source=10.42.100.1, dest=10.42.0.1
			packet: []byte{
				0x45, 0x00, 0x00, 0x28,
				0x00, 0x00, 0x00, 0x00,
				0x40, 0x06, 0x00, 0x00,
				0x0a, 0x2a, 0x64, 0x01, // source: 10.42.100.1
				0x0a, 0x2a, 0x00, 0x01, // dest: 10.42.0.1
			},
			want:    "10.42.100.1",
			wantErr: false,
		},
...
func TestRouterClientLookup(t *testing.T) {
	router := NewRouter("10.42.0.0/16")

	// Add clients
	clients := []Client{
		{ID: "1", Name: "iPhone", PublicKey: "key1", MeshIP: "10.42.100.1"},
		{ID: "2", Name: "Android", PublicKey: "key2", MeshIP: "10.42.100.2"},
	}
	router.UpdateClients(clients)

	// Lookup by IP
	client, ok := router.GetClientByIP("10.42.100.1")
	if !ok {
		t.Fatal("expected to find client for 10.42.100.1")
	}
	...
	// Lookup missing
	_, ok = router.GetClientByIP("10.42.100.99")
	if ok {
		t.Error("expected not to find client for 10.42.100.99")
	}
}
...
func TestRouterIsWGTraffic(t *testing.T) {
	...
	// WG client IP
	if !router.IsWGClientIP("10.42.100.1") {
		t.Error("10.42.100.1 should be WG client IP")
	}

	// Regular mesh IP
	if router.IsWGClientIP("10.42.0.1") {
		t.Error("10.42.0.1 should not be WG client IP")
	}
}
...
func TestRoutePacketFromWG(t *testing.T) {
	...
	// Create a packet from WG client (10.42.100.1) to mesh peer (10.42.0.1)
	packet := []byte{
		0x45, 0x00, 0x00, 0x28,
		0x00, 0x00, 0x00, 0x00,
		0x40, 0x06, 0x00, 0x00,
	...
	if destIP != "10.42.0.1" {
		t.Errorf("expected dest 10.42.0.1, got %s", destIP)
	}
}
...
func TestRoutePacketToWGClient(t *testing.T) {
	...
	// Create a packet from mesh peer (10.42.0.1) to WG client (10.42.100.1)
	packet := []byte{
		0x45, 0x00, 0x00, 0x28,
		0x00, 0x00, 0x00, 0x00,
		0x40, 0x06, 0x00, 0x00,
	...
	if destIP != "10.42.100.1" {
		t.Errorf("expected dest 10.42.100.1, got %s", destIP)
	}
}
...
func TestRoutePacketDropNonWG(t *testing.T) {
	...
	packet := []byte{
		...
		0x40, 0x06, 0x00, 0x00,
		0x0a, 0x2a, 0x00, 0x01, // source: 10.42.0.1
		0x0a, 0x2a, 0x00, 0x02, // dest: 10.42.0.2 (not a WG client)
	}
