...
func TestPeersToTargets(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			peers: []Peer{
				{Name: "peer1", MeshIP: "10.42.0.1", LastSeen: now.Add(-30 * time.Second)},
			},
			expected: 1,
		},
		{
			...
			peers: []Peer{
				{Name: "peer1", MeshIP: "10.42.0.1", LastSeen: now.Add(-5 * time.Minute)},
			},
			expected: 0,
		},
		{
		...
		},
		{
			name: "mixed peers",
			peers: []Peer{
				{Name: "online1", MeshIP: "10.42.0.1", LastSeen: now.Add(-30 * time.Second)},
				{Name: "offline1", MeshIP: "10.42.0.2", LastSeen: now.Add(-5 * time.Minute)},
				{Name: "online2", MeshIP: "10.42.0.3", LastSeen: now.Add(-1 * time.Minute)},
		...
		},
		{
			...
			peers: []Peer{
				{Name: "peer1", MeshIP: "10.42.0.1", LastSeen: now.Add(-threshold)},
			},
		...
		},
		{
			...
			peers: []Peer{
				{Name: "peer1", MeshIP: "10.42.0.1", LastSeen: now.Add(-threshold + time.Second)},
			},
			expected: 1,
		},
	}
...
func TestPeersToTargets_TargetFormat(t *testing.T) {
	now := time.Now()
	peers := []Peer{
		{Name: "test-peer", MeshIP: "10.42.0.42", LastSeen: now},
	}
	...
	if target.Targets[0] != "10.42.0.42:9443" {
		t.Errorf("expected target address '10.42.0.42:9443', got '%s'", target.Targets[0])
	}
...
func TestWriteTargets(t *testing.T) {
	...
	targets := []Target{
		{
			Targets: []string{"10.42.0.1:9443"},
			Labels:  map[string]string{"peer": "peer1"},
		},
		{
			Targets: []string{"10.42.0.2:9443"},
...
func TestGenerator_Generate(t *testing.T) {
	...
	g.SetFetcher(&mockFetcher{
		peers: []Peer{
			...
			{Name: "peer2", MeshIP: "10.42.0.2", LastSeen: now.Add(-5 * time.Minute)}, // offline
		},
	})
...
func TestGenerator_Generate_WriteError(t *testing.T) {
	...
	g.SetFetcher(&mockFetcher{
		peers: []Peer{
			{Name: "peer1", MeshIP: "10.42.0.1", LastSeen: time.Now()},
		},
	})
...
func TestHTTPFetcher_Success(t *testing.T) {
	...
	response := PeersResponse{
		Peers: []Peer{
			{Name: "peer1", MeshIP: "10.42.0.1", LastSeen: now},
			{Name: "peer2", MeshIP: "10.42.0.2", LastSeen: now},
		},
	}
