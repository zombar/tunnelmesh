...
func TestStoreIPAllocation(t *testing.T) {
	...
	// All IPs should be in the WG client range (10.42.100.x - 10.42.199.x)
	for ip := range ips {
		if !isInWGClientRange(ip) {
			t.Errorf("IP %s is not in WG client range (10.42.100.0 - 10.42.199.255)", ip)
		}
	}
}
...
func isInWGClientRange(ip string) bool {
	// WG clients use 10.42.100.0 - 10.42.199.255
	var a, b, c, d int
