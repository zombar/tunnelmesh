...
func TestNewWriter_CustomConfig(t *testing.T) {
	...
	if w.labels["mesh_ip"] != "10.42.0.1" {
		t.Errorf("expected mesh_ip label '10.42.0.1', got %q", w.labels["mesh_ip"])
	}
...
func TestWriter_Flush_SendsCorrectPayload(t *testing.T) {
	...
	if stream.Stream["mesh_ip"] != "10.42.0.5" {
		t.Errorf("expected mesh_ip label '10.42.0.5', got %q", stream.Stream["mesh_ip"])
	}
...
func TestWriter_SetLabels(t *testing.T) {
	...
	if w.labels["mesh_ip"] != "10.42.0.10" {
		t.Errorf("expected mesh_ip label '10.42.0.10', got %q", w.labels["mesh_ip"])
	}
